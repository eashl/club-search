 ...
 *
 * @lastmodified       $Date: 2012-08-01 02:04:26 +0200 (Mi., 01 Aug 2012) $
 *
###
...
(( $, window, document )->
  ...
  lang = if language is null then 'en' else language
  ...
  $(document).ready () ->
    ...
    domReady =
      ...
      init : ->
        ...
        # Functions
        # Hyphenator.run()
        # dr.installSyntaxHighlighting()
        dr.clearForm()

        #$(".fancybox").fancybox()

        # Responsive view
        dr.responsive(dr.getWindowWidth(), false)
        $(window).resize -> dr.responsive(dr.getWindowWidth(), true)
      ...
      navigation : ->
        navHover =
          over : dr.navOverFunction
          out : dr.navOutFunction
          timeout: 200
      ...
      clearForm : ->
        $('#searchForm').submit (event) ->
          ...
          if $('#searchList').length is 0
            $('#searchForm').after('<div id="searchList"></div>')

          #url = $('#searchForm').attr('action') + '?' + $(this).serialize()
          #$('#searchList').load(url + ' #club-search-results-container')
